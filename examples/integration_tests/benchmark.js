...
function main() {
	...
	const signedStakeDelegation = clientJs.signStakeDelegation(
		stakeDelegation,
	...
	new Benchmark.Suite()
		...
		.add(
			"[js]   hashStakeDelegation",
			function () {
				clientJs.hashStakeDelegation(signedStakeDelegation);
			},
		)
		.add(
			"[wasm] hashStakeDelegation",
			function () {
				clientWasm.hashStakeDelegation(signedStakeDelegation);
			},
		)
		.add(
			"[js]   signedRosettaTransactionToSignedCommand - Payment",
			function () {
				clientJs.signedRosettaTransactionToSignedCommand(
					signedRosettaTransactionPayment,
				);
			},
		)
		.add(
			"[wasm] signedRosettaTransactionToSignedCommand - Payment",
			function () {
		...
		.add(
			"[js]   signedRosettaTransactionToSignedCommand - StakeDelegation",
			function () {
				clientJs.signedRosettaTransactionToSignedCommand(
					signedRosettaTransactionStakeDelegation,
				);
			},
		)
		.add(
			"[wasm] signedRosettaTransactionToSignedCommand - StakeDelegation",
			function () {
				clientWasm.signedRosettaTransactionToSignedCommand(
					signedRosettaTransactionStakeDelegation,
				);
